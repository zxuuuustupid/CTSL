...
def plot_ternary_heatmap(csv_path: str, save_path: str = None):
    ...
    # 归一化权重（用于标注与一致性）
    sums = ac + cc + lc
    ac_norm = np.divide(ac, sums, out=np.zeros_like(ac, dtype=float), where=sums != 0)
    cc_norm = np.divide(cc, sums, out=np.zeros_like(cc, dtype=float), where=sums != 0)
    lc_norm = np.divide(lc, sums, out=np.zeros_like(lc, dtype=float), where=sums != 0)
    ...
    # 刻度（真实数值刻度）：基于权重总和推断标注范围
    sum_vals = ac + cc + lc
    sum_min, sum_max = float(np.min(sum_vals)), float(np.max(sum_vals))
    tol = 1e-6 * max(1.0, abs(sum_max))
    if abs(sum_max - sum_min) <= tol:
        tick_total = sum_max  # 近似常和，使用该常数
    else:
        tick_total = np.median(sum_vals)  # 非常和，采用中位数作为参考
