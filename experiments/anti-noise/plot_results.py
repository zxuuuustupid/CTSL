...
def plot_bar_chart(csv_path, method_name="S-MID", save_name=None):
    ...
    # 绘制柱状图
    bars = ax.bar(x, accuracies, width, color=colors, edgecolor='#2c3e50',
                  linewidth=0.8, alpha=0.85, label='Accuracy', zorder=2)

    # 叠加折线图显示趋势（使用深蓝色，与柱状图协调）
    ax.plot(x, accuracies, 'o-', color='#1a5276', linewidth=2,
            markersize=7, markerfacecolor='white', markeredgecolor='#1a5276',
            markeredgewidth=2, label='Trend', zorder=3)
