...
def plot_3d_bar(ax, data, title, x_labels=None, y_labels=None, cmap=None):
    ...
    for i in range(len(dz)):
        if dz[i] > 5:
            ax.text(xpos[i] + dx/2, ypos[i] + dy/2, dz[i] + 3,
                   ...
                   fontsize=14, fontweight='bold', color='#555555')  # 浅灰色
    ...
    ax.set_xlabel('Predicted', fontsize=13, fontweight='bold', labelpad=18, rotation=-15)

    # True (Y轴): 向右倾斜，为了贴合 Y 轴的视觉延伸线
    ax.set_ylabel('True', fontsize=13, fontweight='bold', labelpad=18, rotation=40)
    ...
    ax.set_xticklabels(x_labels, fontsize=10, rotation=30, ha='left', va='center')
    ...
    ax.set_yticklabels(y_labels, fontsize=10, rotation=-30, ha='left', va='center')
    ...
    ax.set_zlabel('Recall (%)', fontsize=16, fontweight='bold', labelpad=6)
    ax.set_zlim(0, 110)
...
def main():
    ...
    for idx, (task_name, pattern, wc) in enumerate(tasks):
        ...
        try:
            ...
            title = task_name
            plot_3d_bar(ax, cm, title, PU_CLASS_NAMES, PU_CLASS_NAMES)

            # 保存单张图
            task_id = idx + 1
            plt.savefig(os.path.join(output_dir, f"Fig_PU_3D_T{task_id}.png"), dpi=300)
            plt.savefig(os.path.join(output_dir, f"Fig_PU_3D_T{task_id}.pdf"), dpi=300)
            plt.savefig(os.path.join(output_dir, f"Fig_PU_3D_T{task_id}.svg"), dpi=300)
            print(f"  已保存: Fig_PU_3D_T{task_id}")
